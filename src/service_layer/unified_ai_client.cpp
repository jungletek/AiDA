...
void UnifiedAIClient::analyze_function(uint64_t address, callback_t callback) {
    // TODO: Get prompt from IDA layer through bridge
    std::string prompt = "Analyze function at address " + std::to_string(address);
    _process_request(prompt, callback, 0.1, "analyze_function");
}

void UnifiedAIClient::suggest_name(uint64_t address, callback_t callback) {
    // TODO: Get prompt from IDA layer through bridge
    std::string prompt = "Suggest name for function at address " + std::to_string(address);
    _process_request(prompt, callback, 0.1, "suggest_name");
}

void UnifiedAIClient::generate_struct(uint64_t address, callback_t callback) {
    // TODO: Get prompt from IDA layer through bridge
    std::string prompt = "Generate struct for function at address " + std::to_string(address);
    _process_request(prompt, callback, 0.1, "generate_struct");
}

void UnifiedAIClient::generate_comment(uint64_t address, callback_t callback) {
    // TODO: Get prompt from IDA layer through bridge
    std::string prompt = "Generate comment for function at address " + std::to_string(address);
    _process_request(prompt, callback, 0.1, "generate_comment");
}

void UnifiedAIClient::generate_hook(uint64_t address, callback_t callback) {
    // TODO: Get prompt from IDA layer through bridge
    std::string prompt = "Generate hook for function at address " + std::to_string(address);
    _process_request(prompt, callback, 0.1, "generate_hook");
}

void UnifiedAIClient::custom_query(uint64_t address, const std::string& question, callback_t callback) {
    // TODO: Get prompt from IDA layer through bridge
    std::string prompt = "Custom query for address " + std::to_string(address) + ": " + question;
    _process_request(prompt, callback, 0.1, "custom_query");
}

void UnifiedAIClient::locate_global_pointer(uint64_t address, const std::string& target_name, addr_callback_t callback) {
    // TODO: Get prompt from IDA layer through bridge
    std::string prompt = "Locate global pointer " + target_name + " from address " + std::to_string(address);
    _process_request(prompt, [callback](const std::string& response) {
        // TODO: Parse response for address through bridge
        // For now, just return the original address
        callback(address);
    }, 0.1, "locate_global_pointer");
}

void UnifiedAIClient::rename_all(uint64_t address, callback_t callback) {
    // TODO: Get prompt from IDA layer through bridge
    std::string prompt = "Rename variables in function at address " + std::to_string(address);
