...
#include <ida.hpp>
#include <typeinf.hpp>
// nlohmann/json.hpp include moved to service layer to avoid circular dependency
...
namespace ida_utils
{
    ...
    std::string get_context_for_prompt(ea_t ea, bool include_struct_context = false, size_t max_len = 0);
    std::string format_context_for_clipboard(const std::string& context_json);
    bool set_clipboard_text(const qstring& text);
    void apply_struct_from_cpp(const std::string& cpp_code, ea_t ea);
    std::string format_prompt(const char* prompt_template, const std::string& context_json);
